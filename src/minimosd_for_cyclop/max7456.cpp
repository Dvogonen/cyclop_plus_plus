...
void Max7456::init(byte iPinCS)
{
  ...
  // There is only one slave. Enable SPI andnever change it again
  pinMode(iPinCS, OUTPUT);
  digitalWrite(iPinCS, LOW);
  delay(100); //power up time

  // Set video to PAL and reset chip
  _regVm0.whole = 0x00;
  ...
  // This seems crazy, the PAL setting above is reset to NTSC!!
  _regVm0.whole = 0x00;
